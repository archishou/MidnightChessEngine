...
#if defined(__AVX512F__)
	auto sum8 = _mm512_hadd_epi32(vec,   vec); // 8 numbers
	auto sum4 = _mm512_hadd_epi32(sum8, sum8); // 4 numbers
	auto sum2 = _mm512_hadd_epi32(sum4, sum4); // 2 numbers

	auto lower_number = _mm512_castsi512_si256(sum2);
	auto higher_number =  _mm512_extracti32x8_epi32(sum2, 1);
	auto result = _mm256_add_epi32(lower_number, higher_number);
...
auto inline veci16_mul_pair_accumi32([[maybe_unused]] auto accum,
									 [[maybe_unused]] auto vec1,
									 [[maybe_unused]] auto vec2) {
#if defined(__AVX512F__)
	return veci32_add(accum, _mm512_madd_epi16(vec1, vec2));
#elif defined(__AVX2__)
	return veci32_add(accum, _mm256_madd_epi16(vec1, vec2));
#elif defined(__ARM_NEON)
	accum = vmlal_s16(accum, vget_low_s16(vec1),  vget_low_s16(vec2));
	accum = vmlal_s16(accum, vget_high_s16(vec1), vget_high_s16(vec2));
	return accum;
#else
	return 0;
#endif
